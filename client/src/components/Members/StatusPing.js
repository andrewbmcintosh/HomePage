...
class StatusPing extends Component {
  ...
  pingLocation = () => {
    const memberId = this.props.memberId;
    axios
      .get(`/api/members/${memberId}/places`)
      ...
      .then(err => {
        const prevLocation = this.state.placesData.filter(
          places =>
  ...
  sendLocationToPlaces = () => {
    ...
    axios
      .post(
        ...
        },${this.state.currentLocation.lng}&key=${API_KEY}`
      )
      .then(res => {
  ...
  render() {
    ...
    return (
      <div>
        <Paper
          onClick={this.initialPingClick}
          className={classes.statusPingClass}
        >
          <Grid
            container
            direction="column"
          ...
          >
            ...
            <Grid item>
              <Typography>{this.props.memberName}</Typography>
            </Grid>
            <Divider variant="middle" />
            <Grid item>
              <Typography>{this.props.membersCurrentStatus}</Typography>
            </Grid>
          </Grid>
        </Paper>

        <div>
          {this.state.pingButtonVisible ? (
            <PingButton
              getAllMembers={this.props.getAllMembers}
              memberId={this.props.memberId}
              currentLocation={this.state.currentLocation}
              placesData={this.state.placesData}
            />
          ) : null}
          {this.state.loadingPingVisible ? <CircularIndeterminate /> : null}
        </div>
      </div>
    );
  }
